...
<div class="panel admin-panel">
    <form action="${ctx}/itemOrder/findBySql2" id="listform" method="post">
    ...
    <c:forEach items="${pagers.datas}" var="data" varStatus="l">
        <tr>
            ...
            <td style="color: red">
                ...
                <c:if test="${data.status == 2||data.status == -1}">待收货</c:if>
                <c:if test="${data.status == 3}">已收货</c:if>
            </td>
            <td>
                <c:if test="${data.status == 0}">
                    <a class="button border-red" href="${ctx}/itemOrder/fh?id=${data.id}"><span class="icon">接单送货</span> </a>
                </c:if>
        ...
        <tr>
            <td colspan="8">
                <div class="pagelist">
                    <!--分页开始-->
                    <pg:pager url="${ctx}/itemOrder/findBySql2" maxIndexPages="5" items="${pagers.total}" maxPageItems="15" export="curPage=pageNumber">
                        <pg:last>
