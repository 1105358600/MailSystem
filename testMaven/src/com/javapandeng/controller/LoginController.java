...
import com.javapandeng.utils.Md5Utils;
...
public class LoginController extends BaseController {
    ...
    public String toLogin(Manage manage, HttpServletRequest request){
        String pwd=Md5Utils.strToMd5(manage.getPassWord());
     ...
     * @return
     */
    @RequestMapping("toDLogin")
    public String toDLogin(Delivery delivery, HttpServletRequest request){
        String pwd=Md5Utils.strToMd5(delivery.getPassWord());
        delivery.setPassWord(pwd);
        Delivery byEntity = deliveryService.getByEntity(delivery);
        if(byEntity==null){
            return "redirect:/login/dtuichu";
        }
        request.getSession().setAttribute(Consts.DELIVERY,byEntity);
        return "/login/dIndex";
    }
     ...
     * 送货员退出
     */
    @RequestMapping("dtuichu")
    public String dtuichu(HttpServletRequest request){
        request.getSession().setAttribute(Consts.DELIVERY,null);
        return "/login/dLogin";
    }
    ...
    public String uIndex(Model model, Item item,HttpServletRequest request){
        ...
        //热销商品
        List<Item> rxs = itemService.listBySqlReturnEntity("select * from item where isDelete=0 order by gmNum desc limit 0,500");
    ...
    public String toRes(User u){
        String pwd=Md5Utils.strToMd5(u.getPassWord());
        u.setPassWord(pwd);
    ...
    public String upass(String password,HttpServletRequest request){
        ...
        Integer userId = Integer.valueOf(attribute.toString());
        User load = userService.load(userId);
        String pwd=Md5Utils.strToMd5(password);
