...
public class ItemOrderController extends BaseController {
    ...
    public String findBySql2(ItemOrder itemOrder, Model model){
        //分页查询
        String sql = "select * from item_order where 1=1 ";
        if(!(isEmpty(itemOrder.getCode()))){
            sql +=" and code like '%"+itemOrder.getCode()+"%' ";
    ...
    public String exAdd(@RequestBody List<CarDto> list,HttpServletRequest request){
        ...
        List<ItemOrder> tlist=itemOrderService.listBySqlReturnEntity("select *  from item_order where status=5");
        if(!tlist.isEmpty()){
            js.put(Consts.RES,3);
            return js.toJSONString();
        }
    ...
    public String zfbpay(Model model,HttpServletRequest request){
        ItemOrder order = new ItemOrder();
        order=itemOrderService.getBySqlReturnEntity("select * from item_order where status=5");
        List<OrderDetail> list= orderDetailService.listBySqlReturnEntity("select * from order_detail where order_id="+order.getId());
        String str="";
        for(int i=0;i<list.size();i++){
            List<Item> ilist= itemService.listBySqlReturnEntity("select * from item where id="+list.get(i).getItemId());
            str+=ilist.get(0).getName()+"  X"+list.get(i).getNum()+"        ";
        }
        model.addAttribute("trade_num",order.getCode());
        model.addAttribute("amounts",order.getTotal());
        model.addAttribute("bodys",str);
        return "Alipay/gopay";};
