        ...
        if (!CollectionUtils.isEmpty(list)){
            for (ItemCategory c : list){
                ...
                if (!CollectionUtils.isEmpty(listBySqlReturnEntity)){
                    ...
                    List<Item> listBySqlReturnEntity2 = itemService.listBySqlReturnEntity("SELECT * FROM item WHERE 1=1 and isDelete =0 and category_id_one="+c.getId());
                    if (!CollectionUtils.isEmpty(listBySqlReturnEntity2)){
                        for (Item i : listBySqlReturnEntity2){
                            tot2+= i.getScNum();
                        }
                }}
